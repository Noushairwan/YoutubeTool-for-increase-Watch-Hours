...
namespace YoutubeTools
{
    public partial class FrmMain : Form
    {
        ...
        const string XPATH_PLAY = "//button[contains(@class, 'ytp-play-button ytp-button')]";
        ...
        private void OpenVideo(string key, bool isMuted)
        {
            ...
            _driver.Navigate().GoToUrl($"{BASE_URL}?v={key}");
            ...
            action.MoveToElement(videoControls).Perform();
            ...
            try
            {
                ...
                {
                    var timeString = GetElement(XPATH_TIME)?.Text?.Trim()?.Split(':');
                    if (timeString != null)
                    {
            ...
            {
                MessageBox.Show($"Error occurred while opening video: {ex.Message}", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }

            dgvWindows.BeginInvoke((MethodInvoker)(() =>
            {
                if (!_isRunning)
                    return;
                ...
                BeginInvoke((MethodInvoker)(() => Text = $"YouTube Tools - {row.Index + 1}"));
            }));
        }

        private IWebElement GetElement(string xPath, int waitSeconds = 20)
        {
            ...
            catch (Exception ex) when (ex is NoSuchElementException || ex is WebDriverTimeoutException || ex is ObjectDisposedException)
            {
                return null;
            }
        }
        ...
        private async Task Start()
        {
            ...
            await Task.Run(async () =>
            {
                ...
                while (_isRunning)
                {
                    ...
                    await Task.Delay(replayCheckInSeconds);
                }
            });
        }
    }
